...
public class ECSClient implements IECSClient,Runnable {
    ...
    String portNum="4321";
    private Collection<ECSNode> inactiveServerNodes;
    private Collection<ECSNode> activeServerNodes;
    private Collection<ECSNode> aliveServerNodes;
    ...
    private void handleCommand(String cmdLine) {
        ...
        } else if(tokens[0].equals("killNode")) {
            if(tokens.length==2){
                Collection<String> namesToRemove=new ArrayList<>();
                namesToRemove.add(tokens[1]);
                killNodes(namesToRemove);
            }else {
                printError("Invalid number of parameters!");
            }
    ...
    private void printHelp() {
        ...
        sb.append("\t removes node with name<serverName>\n");
        sb.append(PROMPT).append("killNode <serverName>");
        sb.append("\t kills node with name<serverName> which triggers data handoff to other servers\n");
    ...
    public boolean start() {
        ...
        while(iter.hasNext()){
            ...
            /*try{
                zkManager.create(getNodePath(ecs),ecs.getNodeMetaData(),
                        ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
                logger.info("Added server:"+getNodePath(ecs));
            }catch (Exception e){
                logger.error("Could not create znode for server",e);
            }*/
        }
        try {
    ...
    public boolean stop() {
        ...
        while(iter.hasNext()){
            ...
            try{
                zkManager.delete(getNodePath(ecs),-1);
            }catch (Exception e){
                logger.error("Could not delete znode for server",e);
            }
        }
        try {
            zkManager.setData(zkRoot, getRootMetaData(activeServerNodes), -1);
    ...
    public IECSNode addNode(String cacheStrategy, int cacheSize) {
        ...
        for(IECSNode node:nodes){
            try{
                zkManager.create(getNodePath(node),node.getNodeMetaData(),
                        ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
                return node;
            }catch (Exception e){
    ...
    public boolean removeNodes(Collection<String> nodeNames) {
        ...
        for(String name: nodeNames){
            ...
            if(ecs!=null) {
                ...
                logger.info("Could not find active server"+name);
            }
        }
        try {
            zkManager.setData(zkRoot, getRootMetaData(activeServerNodes), -1);
    ...
    public boolean killNodes(Collection<String> nodeNames) {
        // TODO
        for(String name: nodeNames){
            ECSNode ecs = activeServerNodes.stream()
                    .filter(node -> name.matches(node.getNodeName()))
            ...
            if(ecs!=null) {
                ...
                aliveServerNodes.remove(ecs);
                try {
                    if (zkManager.exists(getNodePath(ecs), false) == null) {
                        zkManager.delete(getNodePath(ecs), -1);
                    }
                } catch (Exception e) {
                    logger.error("Could not delete znode for server", e);
