        ...
        while (!stop&&clientSocket.isConnected()&&!Thread.interrupted() && !isDisconnected){
            ...
            try {
                ...
                switch (receiveMsg.getStatus()) {
                    case PUT:
                        KVMessage.StatusType status = server.putKVThread(receiveMsg.getKey(), receiveMsg.getValue(), /*replicate=*/false);
                    ...
                    case REPLICATE:
                        ...
                        logger.info("start Replicate client connection");
                        KVMessage.StatusType statusRep = server.putKVThread(receiveMsg.getKey(), receiveMsg.getValue(), /*replicate=*/true);

                        logger.info("finish put Replicate client connection");
                        returnMsg =new ProtoKVMessage(receiveMsg.getKey(),receiveMsg.getValue(),statusRep);
                        try {
                            returnMsg.writeMessage(output);
                            ...
                            stop=true;
                            isDisconnected=true;
                            logger.error("Could not write to replicas, will disconnect",e);
                        }
                        break;
                    case SETUP_ACK:
