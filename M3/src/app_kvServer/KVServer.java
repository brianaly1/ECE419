...
public class KVServer implements IKVServer, Runnable {
	...
	public KVMessage.StatusType putKVThread(String key, String value, boolean isReplicate) throws Exception{
        ...
        if (!inHashRange(key) && !isReplicate) {
        	boolean unlocked = unlockData();
            throw new NotInHashRangeException("Key not in storage range of this kvserver");
        }
		logger.info("putKVThread kvserver checkKeyValueFormat");
		KVMessage.StatusType status = isReplicate ? KVMessage.StatusType.REPLICATE_ERROR : KVMessage.StatusType.PUT_ERROR;
		if(!checkKeyValueFormat(key,value)){
			boolean unlocked = unlockData();
			return status;
		}
		try {
			if(value.isEmpty()) {
				status = isReplicate ? KVMessage.StatusType.REPLICATE_ERROR : KVMessage.StatusType.DELETE_ERROR;
				dataManager.delete(key);
				status = isReplicate ? KVMessage.StatusType.REPLICATE_SUCCESS : KVMessage.StatusType.DELETE_SUCCESS;
				...
				if (dataManager.inStorage(key)) {
					...
					dataManager.update(key, value);
					status = isReplicate ? KVMessage.StatusType.REPLICATE_SUCCESS : KVMessage.StatusType.PUT_UPDATE;
				} else {
					logger.info("putKVThread kvserver putKV,");
					dataManager.putKV(key, value);
					status = isReplicate ? KVMessage.StatusType.REPLICATE_SUCCESS : KVMessage.StatusType.PUT_SUCCESS;
				}
			}
		}catch (Exception e) {
		...
		if (!isReplicate && !noZK) dataManager.serverToReplicasPut(key); 
