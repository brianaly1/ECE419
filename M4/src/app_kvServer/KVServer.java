...
public class KVServer implements IKVServer, Runnable {
	...
	private boolean noZK = true;;

	private ArrayList<Thread> childThreads;
	private ArrayList<Socket> childSockets;
	...
	public Boolean inStorage(String key) throws Exception{
        ...
        if (!inHashRange(key) && !inReplicationRange(key)) {
        	boolean unlocked = unlockData();
	...
	public Boolean inReplicationRange(String key) throws Exception {
		if (noZK) {
			return true;
		}
		return dataManager.inNewReplicationRange(key);
	}
	...
	public KVMessage.StatusType sendToReplicas (String key, String value) throws Exception{
		logger.info("sendToReplicas kvserver lockdata");
		if (!lockData()) {
            throw new ServerDataLockedException("Server data is locked");
        }
		logger.info("sendToReplicas kvserver isActive");
		if (!isActive()) {
			boolean unlocked = unlockData();
			logger.info("is not active");
			throw new ServerNotActiveException("Server not in root metadata");
		}
		logger.info("sendToReplicas kvserver inHashRange");
        if (!inHashRange(key)) {
        	boolean unlocked = unlockData();
            throw new NotInHashRangeException("Key not in hash range of this kvserver");
        }
		logger.info("sendToReplicas kvserver checkKeyValueFormat");
		KVMessage.StatusType status = KVMessage.StatusType.REPLICATE_ERROR;
		if(!checkKeyValueFormat(key,value)){
			boolean unlocked = unlockData();
			return status;
		}
		...
		try {
			dataManager.serverToReplicasPut(key);
			status = KVMessage.StatusType.REPLICATE_SUCCESS;
		} catch (Exception e) {
			System.out.println("Error! Key-Value pair replica handoff failure! \n");
			logger.error("Error! Key-Value pair replica handoff failure! \n", e);
		} finally {
			boolean unlocked = unlockData();
		}
		logger.info("replicate kvserver finish,");
		return status;

	}

	public KVMessage.StatusType putKVThread(String key, String value, boolean isReplicationPut) throws Exception{
		logger.info("putKVThread kvserver lockdata");
		if (!lockData()) {
            throw new ServerDataLockedException("Server data is locked");
        }
		logger.info("putKVThread kvserver isActive");
		if (!isActive()) {
			boolean unlocked = unlockData();
			logger.info("is not active");
			throw new ServerNotActiveException("Server not in root metadata");
		}
		logger.info("putKVThread kvserver inHashRange");
        if (isReplicationPut) {
        	if (!inReplicationRange(key)){
         		boolean unlocked = unlockData();
            	throw new NotInHashRangeException("Key not in replication range of this kvserver");       		
        	}
        } else {
        	if (!inHashRange(key)) {
         		boolean unlocked = unlockData();
		...
		logger.info("putKVThread kvserver checkKeyValueFormat");
		...
		try {
			if(value.isEmpty()) {
				...
				//search for value,
				logger.info("putKVThread kvserver search for value,");
				if (dataManager.inStorage(key)) {
					logger.info("putKVThread kvserver update,");
					dataManager.update(key, value);
					status = KVMessage.StatusType.PUT_UPDATE;
				} else {
					logger.info("putKVThread kvserver putKV,");
					dataManager.putKV(key, value);
		...
		logger.info("putKVThread kvserver finish,");
		return status;
	...
	public void kill(){
		...
		for (Iterator<Socket> iterator = childSockets.iterator(); iterator.hasNext(); ) {
			Socket value = iterator.next();
			try {
				value.close();
				System.out.println("KVServer:Client connection closed:" + value.getInetAddress().getHostName()
						+ " on port " + value.getPort());
				logger.info("Client connection closed:" + value.getInetAddress().getHostName()
						+ " on port " + value.getPort());
			} catch (Exception e){
				System.out.println("KVServer: Could not close client connection:" + value.getInetAddress().getHostName()
						+ " on port " + value.getPort());
				logger.info("Client connection closed:" + value.getInetAddress().getHostName()
						+ " on port " + value.getPort());
			}
